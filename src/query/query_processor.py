...
class QueryProcessor:
    ...
    def _is_auth_error(self, error: Exception | str) -> bool:
        """Return True when an exception looks like an API authentication failure."""
    ...
    def _format_user_error(self, error: Exception | str) -> str:
        """Convert raw backend errors into concise user-facing messages."""

        if self._is_auth_error(error):
            provider = "LLM provider"
            if self.config.llm_base_url.rstrip("/").startswith("https://api.groq.com/openai"):
                provider = "Groq"
            return (
                f"{provider} API key is invalid or expired. "
                "Update DATAWHISPERER_LLM_API_KEY or GROQ_API_KEY and restart the app."
            )
    ...
    def _get_spark_session(self) -> SparkSession:
        ...
        logger.info(
            "Spark session ready with Spark %s using SPARK_HOME '%s'.",
            spark.version,
            getattr(self.config, "spark_home", ""),
        )
        self.spark = spark
        return spark
...
Return corrected SQL:"""
    ...
    def _execute_safely(self, func, *args, timeout_seconds=60, **kwargs):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
            ...
            try:
                ...
                return False, self._format_user_error(e)
    ...
    def _clean_query(self, q: str) -> str:
        ...
        # Remove leading list markers from multi-question inputs without
        # stripping the natural-language meaning of the question itself.
        q = re.sub(r"^\s*(?:[-*]|\d+[.)])\s*", "", q)
        q = re.sub(r"\s+", " ", q)
    ...
    @staticmethod
    def _remove_empty_result_rows(data: pd.DataFrame) -> pd.DataFrame:
        """Drop rows that contain no usable values."""
        ...
        try:
            ...
            if not success:
                return {
                    ...
                    'query': query,
                    'error': result,
                    'query_type': 'fatal' if self._is_auth_error(result) else 'sql_generation'
                }
            ...
            if not success:
                return {
                    ...
                    'query': query,
                    'error': result,
                    'query_type': 'fatal' if self._is_auth_error(result) else 'execution',
                    'sql': sql_query
                }
                
            result_df = result
            result_pandas = self._remove_empty_result_rows(result_df.toPandas())

            if result_pandas.empty:
                return {
                    'success': False,
                    'query': query,
                    'error': 'Query returned no usable results. Try rephrasing your question or choosing columns with non-empty values.',
            ...
            return {
                ...
                'query': query,
                'error': error_msg,
                'query_type': 'fatal' if self._is_auth_error(e) else 'unexpected_error'
            }
